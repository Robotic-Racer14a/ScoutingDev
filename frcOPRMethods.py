...
API_KEY = "3qyocWoT5apgRcKo5mL2epJ7c5tfhlWuHRQYY9kDxmhLEX078hPBz5UEw7hL16IT"
...
def print_match_options():
    ...
    print(matches[0]["score_breakdown"]["red"])
...
def get_event_matches_alliance_scores(criteras):
    ...
    for critera in criteras:
        ...
        for match in matches:
            if "qm" in match["key"][len(event_key) + 1:] or include_elims:
                critera_list.append({
                    "red_teams": match["alliances"]["red"]["team_keys"],
                    "red_score": match["score_breakdown"]["red"][critera],
                    "blue_teams": match["alliances"]["blue"]["team_keys"],
                    "blue_score": match["score_breakdown"]["blue"][critera],
                    "match_key": match["key"][len(event_key) + 1:]
                    })
...
def get_event_matches_team_objectives(criteras):
    ...
    for critera in criteras:
        ...
        for match in matches:
            if "qm" in match["key"][len(event_key) + 1:] or include_elims:
                critera_list.append({
                    "red_one": (match["alliances"]["red"]["team_keys"][0], match["score_breakdown"]["red"][f"{critera}Robot1"]),
                    "red_two": (match["alliances"]["red"]["team_keys"][1], match["score_breakdown"]["red"][f"{critera}Robot2"]),
                    "red_three": (match["alliances"]["red"]["team_keys"][2], match["score_breakdown"]["red"][f"{critera}Robot3"]),
                    "blue_one": (match["alliances"]["blue"]["team_keys"][0], match["score_breakdown"]["blue"][f"{critera}Robot1"]),
                    "blue_two": (match["alliances"]["blue"]["team_keys"][1], match["score_breakdown"]["blue"][f"{critera}Robot2"]),
                    "blue_three": (match["alliances"]["blue"]["team_keys"][2], match["score_breakdown"]["blue"][f"{critera}Robot3"]),
                    "match_key": match["key"][len(event_key) + 1:],
                    })
        matchList[critera] = critera_list
    return matchList
...
def calculate_team_completion(matches, teams, correct_keys):
    ...
    for match in matches:
        for key in alliance_keys:
            if key in match:
                ...
                if team_num in team_totals:
                    if (team_totals[team_num] < point_score): team_totals[team_num] = point_score
